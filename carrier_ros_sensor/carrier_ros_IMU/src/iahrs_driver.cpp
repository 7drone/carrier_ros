...
#include "carrier_ros_imu/euler_angle_init.h"
...
carrier_ros_imu::all_data_reset all_data_reset_cmd;
ros::ServiceServer euler_angle_init_service;
carrier_ros_imu::euler_angle_init euler_angle_init_cmd;
ros::ServiceServer euler_angle_reset_service;
carrier_ros_imu::euler_angle_reset euler_angle_reset_cmd;
ros::ServiceServer pose_velocity_reset_service;
carrier_ros_imu::pose_velocity_reset pose_velocity_reset_cmd;
ros::ServiceServer reboot_sensor_service;
carrier_ros_imu::reboot_sensor reboot_sensor_cmd;
...
bool All_Data_Reset_Command(carrier_ros_imu::all_data_reset::Request  &req, 
					    	carrier_ros_imu::all_data_reset::Response &res)
{
	bool bResult = false;
...
bool Euler_Angle_Init_Command(carrier_ros_imu::euler_angle_init::Request  &req, 
					    	  carrier_ros_imu::euler_angle_init::Response &res)
{
	bool bResult = false;
...
bool Euler_Angle_Reset_Command(carrier_ros_imu::euler_angle_reset::Request  &req, 
					    	   carrier_ros_imu::euler_angle_reset::Response &res)
{
	bool bResult = false;
...
bool Pose_Velocity_Reset_Command(carrier_ros_imu::pose_velocity_reset::Request  &req, 
					    	     carrier_ros_imu::pose_velocity_reset::Response &res)
{
	bool bResult = false;
...
bool Reboot_Sensor_Command(carrier_ros_imu::reboot_sensor::Request  &req, 
					       carrier_ros_imu::reboot_sensor::Response &res)
{
	bool bResult = false;
...
int main (int argc, char** argv)
{
	signal(SIGINT,my_handler);
    	ros::init(argc, argv, "carrier_ros_imu");
	...
	ros::Publisher imu_magneticfield_pub = nh.advertise<sensor_msgs::MagneticField>("imu/mag",1);
	//IMU Service///////////////////////////////////////////////////////////////////////////////////////////////
	ros::NodeHandle sh;
    ...
    while(ros::ok())
    {
		...
		if (serial_fd >= 0) 
		{
			...
			if(no_data >= 3)
			{
				magnetic_data_msg.x = magneticfield_msg.magnetic_field.x = data[0];
				magnetic_data_msg.y = magneticfield_msg.magnetic_field.y = data[1];
				magnetic_data_msg.z = magneticfield_msg.magnetic_field.z = data[2];
			}
			...
			imu_data_msg.header.stamp = magneticfield_msg.header.stamp = measurement_time;
			imu_data_msg.header.frame_id = magneticfield_msg.header.frame_id = tf_prefix_ + "imu_link";  // "imu_link"

			// publish the IMU data
			imu_data_pub.publish(imu_data_msg);
			imu_magnetic_pub.publish(magnetic_data_msg);
			imu_magneticfield_pub.publish(magneticfield_msg);
			//Publish tf

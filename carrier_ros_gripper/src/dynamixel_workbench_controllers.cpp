...
void DynamixelController::initSubscriber()
{
  ...
  if (is_cmd_vel_topic_) cmd_vel_sub_ = priv_node_handle_.subscribe("/dynamixel/cmd_vel", 10, &DynamixelController::commandVelocityCallback, this);
}
...
//       = r * ((RPM * Goal_Velocity) * 0.10472)		=> Goal_Velocity = V / (r * RPM * 0.10472) = V * VELOCITY_CONSTATNE_VALUE
  ...
  wheel_velocity[motor] = robot_lin_vel + (robot_ang_vel * wheel_separation_ / 2);

  if (dxl_wb_->getProtocolVersion() == 2.0f)
  {
    if (strcmp(dxl_wb_->getModelName(id_array[0]), "XL-320") == 0)
    {
      if (wheel_velocity[motor] == 0.0f) dynamixel_velocity[motor] = 0;
      else if (wheel_velocity[motor] < 0.0f) dynamixel_velocity[motor] = ((-1.0f) * wheel_velocity[motor]) * velocity_constant_value + 1023;
      else if (wheel_velocity[motor] > 0.0f) dynamixel_velocity[motor] = (wheel_velocity[motor] * velocity_constant_value);
    }
    else
    {
      dynamixel_velocity[motor]  = wheel_velocity[motor] * velocity_constant_value;
    }
  }
  else if (dxl_wb_->getProtocolVersion() == 1.0f)
  {
